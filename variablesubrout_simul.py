...
## Purpose      : runs the simulation for variable subroute machine
#################################################################################
...
def createtrj(min_patterns, kwargs):
    """
        This fucntion calls the process_edgemsg() function to simulate variabel node subroutine
    ...
    with ProcessPoolExecutor(
        ...
        futures  = []
        paramlist = [(im, m_, kwargs.inputsze, kwargs.flushsze, "variable") for im, m_ in enumerate(min_patterns)]
...
def createstochmat(dirargs, kwargs):
    """
        This function uses helper functions to define state space of the variable node subroutine computation simulated, 
    ...
    nsp_state2idx = creatensplookupdir(state_to_idx = state2idx, _nsplookuppath = _nsplookuppath, _machine = "variable")
    ...
    with ProcessPoolExecutor(
        ...
        for fp in edgefiles:
            ex.submit(collect_transitions_nsp, fp, dirargs.get("nsptrans_dir"), "variable")
...
def runmmc(_initstate_idx, _nsp_state2idx, dirargs, kwargs):
    tic_ = time.time()
    pmmc_subr(_initstate_idx, _nsp_state2idx, dirargs, kwargs)
